    ...
    implements DeviceSelectionListener, ConnectionCallbacks, OnConnectionFailedListener, OnFailedListener
{
   ...
   * {@link BaseCastManager} but each subclass may have some additional logic that can be done, e.g. detaching data or media channels that they may
   * have set up.
   */
   ...
   * @return
   */
  public abstract Cast.CastOptions.Builder getCastOptionBuilder(CastDevice device);
   ...
   * @param wasLaunched
   */
  public abstract void onApplicationConnected(ApplicationMetadata applicationMetadata, String applicationStatus, String sessionId, boolean wasLaunched);
   ...
   * @param statusCode
   */
  public abstract void onApplicationConnectionFailed(int statusCode);
   ...
   * @param statusCode
   */
  public abstract void onApplicationStopFailed(int statusCode);
  ...
  /**
   * This is called from {@link com.google.sample.castcompanionlibrary.cast.CastMediaRouterCallback} to signal the change in presence of cast devices
   * on network.
   * 
   * @param castPresent
   */
  ...
  public void setDevice(CastDevice device)
  {
    ...
    if (null == mApiClient)
    {
      ...
      mApiClient = new GoogleApiClient.Builder(mContext).addApi(Cast.API, apiOptionsBuilder.build()).addConnectionCallbacks(this).addOnConnectionFailedListener(
          this).build();
      mApiClient.connect();
    }
   ...
   * assumed that the enclosing {@link android.app.Activity} inherits (directly or indirectly) from {@link android.support.v7.app.ActionBarActivity}.
   * 
   * @param menu
   ...
   * <ul>
   * <li>The enclosing {@link android.app.Activity} inherits (directly or indirectly) from {@link android.support.v4.app.FragmentActivity}</li>
  ...
  /**
   * Clears the {@link android.content.Context}. Should be used when the client application is being destroyed to avoid context leak.
   */
  ...
  /**
   * Clears the {@link android.content.Context} if the current context is the same as the one provided in the argument <code>context</code>. Should be
   * used when the client application is being destroyed to avoid context leak.
   * 
   * @param context
   */
      ...
      throws TransientNetworkDisconnectionException, NoConnectionException
  {
    ...
    if (mReconnectionStatus == ReconnectionStatus.IN_PROGRESS)
    {
      ...
      Cast.CastApi.joinApplication(mApiClient, mApplicationId, sessionId).setResultCallback(new ResultCallback<Cast.ApplicationConnectionResult>()
      {
    ...
    else
    {
      ...
      Cast.CastApi.launchApplication(mApiClient, mApplicationId).setResultCallback(new ResultCallback<Cast.ApplicationConnectionResult>()
      {
