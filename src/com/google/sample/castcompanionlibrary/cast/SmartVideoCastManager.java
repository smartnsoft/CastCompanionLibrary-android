...
import com.google.sample.castcompanionlibrary.cast.dialog.video.SmartVideoMediaRouteDialogFactory;
...
/**
 * A concrete subclass of {@link BaseCastManager} that is suitable for casting video contents (it also provides a single custom data channel/namespace
 * if an out-of-bound communication is needed).
 * <p>
 * This is a singleton that needs to be "initialized" (by calling <code>initialize()</code>) prior to usage. Subsequent to initialization, an easier
 * way to get access to the singleton class is to call a variant of <code>getInstance()</code>. After initialization, callers can enable a number of
 * features (all features are off by default). To do so, call <code>enableFeature()</code> and pass an OR-ed expression built from one ore more of the
 * following constants:
 * <p>
 * <ul>
 ...
 * </ul>
 * Callers can add {@link MiniController} components to their application pages by adding the corresponding widget to their layout xml and then
 * calling <code>addMiniController()</code>. This class manages various states of the remote cast device.Client applications, however, can complement
 * the default behavior of this class by hooking into various callbacks that it provides (see {@link IVideoCastConsumer}). Since the number of these
 * callbacks is usually much larger than what a single application might be interested in, there is a no-op implementation of this interface (see
 * {@link VideoCastConsumerImpl}) that applications can subclass to override only those methods that they are interested in. Since this library
 * depends on the cast functionalities provided by the Google Play services, the library checks to ensure that the right version of that service is
 * installed. It also provides a simple static method <code>checkGooglePlayServices()</code> that clients can call at an early stage of their
 * applications to provide a dialog for users if they need to update/activate their GMS library. To learn more about this library, please read the
 * documentation that is distributed as part of this library.
 */
public class SmartVideoCastManager
    extends SmartBaseCastManager
    implements OnMiniControllerChangedListener, OnFailedListener
{
  ...
  /**
   * Volume can be controlled at two different layers, one is at the "stream" level and one at the "device" level. <code>VolumeType</code>
   * encapsulates these two types.
   */
  ...
  private static final String TAG = LogUtils.makeLogTag(SmartVideoCastManager.class);

  private static SmartVideoCastManager sInstance;
  ...
  private Set<IVideoCastConsumer> mVideoConsumers = Collections.synchronizedSet(new HashSet<IVideoCastConsumer>());
  ...
  /**
   * Initializes the VideoCastManager for clients. Before clients can use VideoCastManager, they need to initialize it by calling this static method.
   * Then clients can obtain an instance of this singleton class by calling {@link getInstance()} or {@link getInstance(Context)}. Failing to
   * initialize this class before requesting an instance will result in a {@link CastException} exception.
   *
   * @param context
   * @param applicationId
   *          the unique ID for your application
   * @param targetActivity
   *          this points to the activity that should be invoked when user clicks on the icon in the {@link MiniController}. Often this is the
   *          activity that hosts the local player.
   * @param dataNamespace
   *          if not <code>null</code>, a custom data channel with this namespace will be created.
   * @return
   * @see getInstance()
   */
  public static synchronized SmartVideoCastManager initialize(Context context, String applicationId, Class<?> targetActivity, String dataNamespace)
  {
    if (null == sInstance)
    {
      ...
      sInstance = new SmartVideoCastManager(context, applicationId, targetActivity, dataNamespace);
  ...
  {
    if (isFeatureEnabled(BaseCastManager.FEATURE_CAPTIONS_PREFERENCE))
    {
  ...
  /**
   * Returns the initialized instances of this class. If it is not initialized yet, a {@link CastException} will be thrown.
   *
   * @return
   * @throws CastException
   * @see initialize()
   */
  public static SmartVideoCastManager getInstance()
      throws CastException
  {
    if (null == sInstance)
    {
  ...
  /**
   * Returns the initialized instances of this class. If it is not initialized yet, a {@link CastException} will be thrown. The {@link Context} that
   * is passed as the argument will be used to update the context for the <code>VideoCastManager</code> instance. The main purpose of updating context
   * is to enable the library to provide {@link Context} related functionalities, e.g. it can create an error dialog if needed. This method is
   * preferred over the similar one without a context argument.
   *
   * @param context
   *          the current Context
   * @return
   ...
   */
  public static SmartVideoCastManager getInstance(Context context)
      throws CastException
  {
    if (null == sInstance)
    {
      LOGE(TAG, "No VideoCastManager instance was built, you need to build one first " + "(called from Context: " + context + ")");
      throw new CastException();
    }
  ...
  private SmartVideoCastManager(Context context, String applicationId, Class<?> targetActivity, String dataNamespace)
  {
    ...
    Utils.saveStringToPreference(mContext, BaseCastManager.PREFS_KEY_CAST_ACTIVITY_NAME, mTargetActivity.getName());
    if (null != mDataNamespace)
    {
      Utils.saveStringToPreference(mContext, BaseCastManager.PREFS_KEY_CAST_CUSTOM_DATA_NAMESPACE, dataNamespace);
    }
   ...
   * (non-Javadoc)
   * 
   * @see com.google.sample.castcompanionlibrary.widgets.MiniController. OnMiniControllerChangedListener#onPlayPauseClicked(android.view.View)
   * 
   * @throws TransientNetworkDisconnectionException
   * 
   * @throws NoConnectionException
   ...
   * (non-Javadoc)
   * 
   * @see com.google.sample.castcompanionlibrary.widgets.MiniController. OnMiniControllerChangedListener
   * #onTargetActivityInvoked(android.content.Context)
   */
  @Override
  public void onTargetActivityInvoked(Context ctx)
      throws TransientNetworkDisconnectionException, NoConnectionException
  {
  ...
  /**
   * Updates the visibility of the mini controllers. In most cases, clients do not need to use this as the {@link SmartVideoCastManager} handles the
   * visibility.
   *
   * @param visible
   */
   ...
   * @param context
   * @param mediaWrapper
   *          a bundle wrapper for the media that is or will be casted
   * @param position
   *          (in milliseconds) is the starting point of the media playback
   * @param shouldStart
   *          indicates if the remote playback should start after launching the new page
   * @param customData
   *          Optional {@link JSONObject}
   */
  ...
  /**
   * Launches the {@link VideoCastControllerActivity} that provides a default Cast Player page. This variation should be used when an
   * {@link IMediaAuthService} needs to be used.
   *
   * @param context
   * @param authService
   */
   ...
   * @param ctx
   * @param mediaInfo
   *          pointing to the media that is or will be casted
   * @param position
   *          (in milliseconds) is the starting point of the media playback
   * @param shouldStart
   *          indicates if the remote playback should start after launching the new page
  ...
  /**
   * Returns the active {@link RemoteMediaPlayer} instance. Since there are a number of media control APIs that this library do not provide a wrapper
   * for, client applications can call those methods directly after obtaining an instance of the active {@link RemoteMediaPlayer}.
   *
   * @return
   */
  ...
  /**
   * A helper method to determine if, given a player state and an idle reason (if the state is idle) will warrant having a UI for remote presentation
  ...
  /**
   * Returns the url for the movie that is currently playing on the remote device. If there is no connection, this will return <code>null</code>.
   *
   * @return
   * @throws NoConnectionException
   *           If no connectivity to the device exists
   * @throws TransientNetworkDisconnectionException
   *           If framework is still trying to recover from a possibly transient loss of network
   */
  ...
  /**
   * Returns <code>true</code> only if there is a media on the remote being played, paused or buffered.
   *
   * @return
   * @throws NoConnectionException
   * @throws TransientNetworkDisconnectionException
  ...
  /**
   * Gets the remote's system volume. If no device is connected to, or if an exception is thrown, this returns -1. It internally detects what type of
   * volume is used.
   *
   * @throws NoConnectionException
   *           If no connectivity to the device exists
  ...
  /**
   * Sets the volume. It internally determines if this should be done for <code>stream</code> or <code>device</code> volume.
   *
   * @param volume
   ...
   * @throws TransientNetworkDisconnectionException
   * @throws CastException
   *           If setting system volume fails
   */
      ...
      throws CastException, TransientNetworkDisconnectionException, NoConnectionException
  {
    ...
    {
      checkRemoteMediaPlayerAvailable();
      mRemoteMediaPlayer.setStreamVolume(mApiClient, volume).setResultCallback(new ResultCallback<MediaChannelResult>()
      {
        ...
        public void onResult(MediaChannelResult result)
        {
          ...
          {
            onFailed(R.string.failed_setting_volume, result.getStatus().getStatusCode());
          }
        }
      });
    }
  ...
  /**
   * Increments (or decrements) the volume by the given amount. It internally determines if this should be done for <code>stream</code> or
  ...
  /**
   * Increments or decrements volume by <code>delta</code> if <code>delta &gt; 0</code> or <code>delta &lt; 0</code>, respectively. Note that the
   * volume range is between 0 and {@link RouteInfo.getVolumeMax()}.
   *
   * @param delta
   */
  ...
  /**
   * Returns <code>true</code> if remote device is muted. It internally determines if this should be done for <code>stream</code> or
   * <code>device</code> volume.
   *
   * @return
   * @throws NoConnectionException
  ...
  /**
   * Mutes or un-mutes the volume. It internally determines if this should be done for <code>stream</code> or <code>device</code> volume.
   *
   * @param mute
   * @throws CastException
   * @throws NoConnectionException
  ...
  /**
   * Returns the time left (in milliseconds) of the current media. If there is no {@code RemoteMediaPlayer}, it returns -1.
   *
   * @throws TransientNetworkDisconnectionException
   * @throws NoConnectionException
   */
      ...
      throws TransientNetworkDisconnectionException, NoConnectionException
  {
    ...
    return isRemoteStreamLive() ? mLiveStreamDuration : mRemoteMediaPlayer.getStreamDuration() - mRemoteMediaPlayer.getApproximateStreamPosition();
  }
  ...
  /*
   * Starts a service that can last beyond the lifetime of the application to provide notifications. The service brings itself down when needed. The
   * service will be started only if the notification feature has been enabled during the initialization.
   * 
   * @see {@link BaseCastManager#enableFeatures()}
   */
  private boolean startNotificationService(boolean visibility)
  {
    if (!isFeatureEnabled(BaseCastManager.FEATURE_NOTIFICATION))
    {
      return true;
    }
  ...
  private void onApplicationDisconnected(int errorCode)
  {
    ...
    if (null != mRemoteControlClientCompat && isFeatureEnabled(BaseCastManager.FEATURE_LOCKSCREEN))
    {
  ...
  @Override
  void onApplicationConnected(ApplicationMetadata appMetadata, String applicationStatus, String sessionId, boolean wasLaunched)
  {
    ...
    try
    {
      ...
      Utils.saveStringToPreference(mContext, BaseCastManager.PREFS_KEY_SESSION_ID, mSessionId);
      mRemoteMediaPlayer.requestStatus(mApiClient).setResultCallback(new ResultCallback<MediaChannelResult>()
      {
        ...
        public void onResult(MediaChannelResult result)
        {
          if (!result.getStatus().isSuccess())
          {
            onFailed(R.string.failed_status_request, result.getStatus().getStatusCode());
          }
    ...
    catch (TransientNetworkDisconnectionException e)
    {
      ...
      onFailed(R.string.failed_no_connection_trans, BaseCastManager.NO_STATUS_CODE);
    }
    catch (NoConnectionException e)
    {
      LOGE(TAG, "Failed to attach media/data channel due to network issues", e);
      onFailed(R.string.failed_no_connection, BaseCastManager.NO_STATUS_CODE);
    }
    ...
    else
    {
      ...
      if (showError)
      {
        ...
        case CastStatusCodes.APPLICATION_NOT_FOUND:
          LOGD(TAG, "onApplicationConnectionFailed(): failed due to: " + "ERROR_APPLICATION_NOT_FOUND");
        ...
        case CastStatusCodes.TIMEOUT:
          ...
          break;
        default:
          LOGD(TAG, "onApplicationConnectionFailed(): failed due to: errorcode=" + errorCode);
   ...
   * @param media
   * @param autoPlay
   *          If <code>true</code>, playback starts after load
   * @param position
   *          Where to start the playback (only used if autoPlay is <code>true</code>. Units is milliseconds.
   * @throws NoConnectionException
   * @throws TransientNetworkDisconnectionException
   */
   ...
   *          Where to start the playback (only used if autoPlay is <code>true</code>). Units is milliseconds.
   * @param customData
   *          Optional {@link JSONObject} data to be passed to the cast device
   * @throws NoConnectionException
   * @throws TransientNetworkDisconnectionException
   */
   ...
   * @param media
   * @param activeTracks
   *          An array containing the list of track IDs to be set active for this media upon a successful load
   * @param autoPlay
   *          If <code>true</code>, playback starts after load
   * @param position
   *          Where to start the playback (only used if autoPlay is <code>true</code>). Units is milliseconds.
   ...
   * @throws TransientNetworkDisconnectionException
   */
  public void loadMedia(MediaInfo media, final long[] activeTracks, boolean autoPlay, int position, JSONObject customData)
      throws TransientNetworkDisconnectionException, NoConnectionException
  {
    LOGD(TAG, "loadMedia");
    checkConnectivity();
    ...
    mRemoteMediaPlayer.load(mApiClient, media, autoPlay, position, customData).setResultCallback(new ResultCallback<MediaChannelResult>()
    {
   ...
   *
   * @param position
   *          Where to start the playback. Units is milliseconds.
   * @throws NoConnectionException
   * @throws TransientNetworkDisconnectionException
   */
   ...
   * @throws NoConnectionException
   */
  public void stop(JSONObject customData)
      throws TransientNetworkDisconnectionException, NoConnectionException
  {
    LOGD(TAG, "stop()");
    checkConnectivity();
      ...
      throws TransientNetworkDisconnectionException, NoConnectionException
  {
    ...
    mRemoteMediaPlayer.seek(mApiClient, position, RemoteMediaPlayer.RESUME_STATE_UNCHANGED).setResultCallback(new ResultCallback<MediaChannelResult>()
    {
      ...
      throws TransientNetworkDisconnectionException, NoConnectionException
  {
    ...
    mRemoteMediaPlayer.seek(mApiClient, position, RemoteMediaPlayer.RESUME_STATE_PLAY).setResultCallback(resultCallback);
  }
    ...
    if (null == mRemoteMediaPlayer)
    {
      ...
      mRemoteMediaPlayer.setOnStatusUpdatedListener(new RemoteMediaPlayer.OnStatusUpdatedListener()
      {
        ...
        public void onStatusUpdated()
        {
          ...
          SmartVideoCastManager.this.onRemoteMediaPlayerStatusUpdated();
        }
      });
      ...
      mRemoteMediaPlayer.setOnMetadataUpdatedListener(new RemoteMediaPlayer.OnMetadataUpdatedListener()
      {
        ...
        public void onMetadataUpdated()
        {
          ...
          SmartVideoCastManager.this.onRemoteMediaPlayerMetadataUpdated();
        }
      });
  ...
  /**
   * Returns the Idle reason, defined in <code>MediaStatus.IDLE_*</code>. Note that the returned value is only meaningful if the status is truly
  ...
  /*
   * If a data namespace was provided when initializing this class, we set things up for a data channel
   * 
   * @throws NoConnectionException
   * 
   * @throws TransientNetworkDisconnectionException
  ...
  /**
   * Sends the <code>message</code> on the data channel for the namespace that was provided during the initialization of this class. If
   * <code>messageId &gt; 0</code>, then it has to be a unique identifier for the message; this id will be returned if an error occurs. If
   * <code>messageId == 0</code>, then an auto-generated unique identifier will be created and returned for the message.
   *
   * @param message
   * @throws IllegalStateException
   *           If the namespace is empty or null
   * @throws NoConnectionException
   *           If no connectivity to the device exists
   * @throws TransientNetworkDisconnectionException
   *           If framework is still trying to recover from a possibly transient loss of network
      ...
      throws TransientNetworkDisconnectionException, NoConnectionException
  {
    ...
    Cast.CastApi.sendMessage(mApiClient, mDataNamespace, message).setResultCallback(new ResultCallback<Status>()
    {
      ...
      public void onResult(Status result)
      {
        if (!result.isSuccess())
        {
          SmartVideoCastManager.this.onMessageSendFailed(result.getStatusCode());
        }
      }
    });
  }
  ...
  /**
   * Remove the custom data channel, if any. It returns <code>true</code> if it succeeds otherwise if it encounters an error or if no connection
   * exists or if no custom data channel exists, then it returns <code>false</code>
   *
   * @return
   */
  public boolean removeDataChannel()
  {
    ...
    try
    {
      ...
      mDataChannel = null;
      Utils.saveStringToPreference(mContext, BaseCastManager.PREFS_KEY_CAST_CUSTOM_DATA_NAMESPACE, null);
      return true;
    }
    catch (Exception e)
    {
  ...
  private void onRemoteMediaPlayerStatusUpdated()
  {
    ...
    if (null == mApiClient || null == mRemoteMediaPlayer || null == mRemoteMediaPlayer.getMediaStatus())
    {
    ...
    try
    {
      ...
      else if (mState == MediaStatus.PLAYER_STATE_IDLE)
      {
        ...
        case MediaStatus.IDLE_REASON_ERROR:
          ...
          makeUiHidden = true;
          removeRemoteControlClient();
          onFailed(R.string.failed_receiver_player_error, BaseCastManager.NO_STATUS_CODE);
          break;
      ...
      synchronized (mVideoConsumers)
      {
        for (IVideoCastConsumer consumer : mVideoConsumers)
        {
          ...
          {
            LOGE(TAG, "onRemoteMediaPlayerStatusUpdated(): Failed to inform " + consumer, e);
          }
        }
      }
    }
  ...
  /*
   * Sets up the {@link RemoteControlClient} for this application. It also handles the audio focus.
   */
  ...
  /*
   * Returns the {@link Bitmap} appropriate for the right size image for lock screen. In ICS and JB, the image shown on the lock screen is a small
   * size bitmap but for KitKat, the image is a full-screen image so we need to separately handle these two cases. Should not be called on the main
   * thread.
   */
  private void setBitmapForLockScreen(MediaInfo video)
  {
    ...
    if (null != bm)
    {
      mRemoteControlClientCompat.editMetadata(false).putBitmap(RemoteControlClientCompat.MetadataEditorCompat.METADATA_KEY_ARTWORK, bm).apply();
    }
    else
    {
      ...
      new FetchBitmapTask()
      {
        ...
        {
          mRemoteControlClientCompat.editMetadata(false).putBitmap(RemoteControlClientCompat.MetadataEditorCompat.METADATA_KEY_ARTWORK, bitmap).apply();
        }
      }.start(imgUrl);
    }
  }
  ...
  /*
   * On ICS and JB, lock screen metadata is one liner: Title - Album Artist - Album. On KitKat, it has two lines: Title , Album Artist - Album
   */
  private void updateLockScreenMetadata()
  {
    ...
    try
    {
      ...
      mRemoteControlClientCompat.editMetadata(false).putString(MediaMetadataRetriever.METADATA_KEY_TITLE, mm.getString(MediaMetadata.KEY_TITLE)).putString(
          MediaMetadataRetriever.METADATA_KEY_ALBUMARTIST, mContext.getResources().getString(R.string.casting_to_device, getDeviceName())).putLong(
          MediaMetadataRetriever.METADATA_KEY_DURATION, info.getStreamDuration()).apply();
    }
  ...
  /**
   * Registers an {@link IVideoCastConsumer} interface with this class. Registered listeners will be notified of changes to a variety of lifecycle and
   * media status changes through the callbacks that the interface provides.
   *
   * @param listener
  ...
  /**
   * Adds a new {@link IMiniController} component. Callers need to provide their own {@link OnMiniControllerChangedListener}.
   *
  ...
  public void addMiniController(IMiniController miniController, OnMiniControllerChangedListener onChangedListener)
  {
    if (null != miniController)
    {
      ...
      {
        LOGD(TAG, "Attempting to adding " + miniController + " but it was already " + "registered, skipping this step");
      }
    }
  }

  /**
   * Adds a new {@link IMiniController} component and assigns {@link SmartVideoCastManager} as the {@link OnMiniControllerChangedListener} for this
   * component.
   *
   * @param miniController
   */
  ...
  Builder getCastOptionBuilder(CastDevice device)
  {
    ...
    if (isFeatureEnabled(BaseCastManager.FEATURE_DEBUGGING))
    {
  ...
  {
    return new SmartVideoMediaRouteDialogFactory();
  }
      ...
      extends Cast.Listener
  {
    ...
    {
      SmartVideoCastManager.this.onApplicationDisconnected(statusCode);
    }
    ...
    {
      SmartVideoCastManager.this.onApplicationStatusChanged();
    }

    @Override
    public void onVolumeChanged()
    {
      SmartVideoCastManager.this.onVolumeChanged();
    }
  }
  ...
  /**
   * Clients can call this method to delegate handling of the volume. Clients should override {@link dispatchEvent} and call this method:
   * 
   * <pre>
   ...
   * @param volumeDelta
   *          The amount by which volume should be increased or decreased in each step
  ...
  {
    if ((Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN) && getPlaybackStatus() == MediaStatus.PLAYER_STATE_PLAYING && isFeatureEnabled(BaseCastManager.FEATURE_LOCKSCREEN))
    {
      return false;
    }
  ...
  /**
   * Set the live stream duration; this is purely used in the reconnection logic. If this method is not called, the default value
   * {@code DEFAULT_LIVE_STREAM_DURATION_MS} is used.
   *
   * @param duration
  ...
  public void setActiveTrackIds(long[] trackIds)
  {
    ...
    mRemoteMediaPlayer.setActiveMediaTracks(mApiClient, trackIds).setResultCallback(new ResultCallback<MediaChannelResult>()
    {
      @Override
      public void onResult(MediaChannelResult mediaChannelResult)
      {
        ...
        {
          LOGD(TAG, "Failed since: " + mediaChannelResult.getStatus() + " and status code:" + mediaChannelResult.getStatus().getStatusCode());
        }
      }
    });
  }
  ...
  public void setTextTrackStyle(TextTrackStyle style)
  {
    mRemoteMediaPlayer.setTextTrackStyle(mApiClient, style).setResultCallback(new ResultCallback<MediaChannelResult>()
    {
      ...
      public void onResult(MediaChannelResult result)
      {
        if (!result.getStatus().isSuccess())
        {
          onFailed(R.string.failed_to_set_track_style, result.getStatus().getStatusCode());
        }
      }
    });
    synchronized (mVideoConsumers)
  ...
  /**
   * Updates the summary of the captions between "on" and "off" based on the user selected preferences. This can be called by the caller application
   * when they add captions settings to their preferences. Preferably this should be called in the {@link onResume()} of the PreferenceActivity so
   * that it gets updated when needed.
   */
  ...
  /**
   * Returns the list of current active tracks. If there is no remote media, then this will return <code>null</code>.
   */
