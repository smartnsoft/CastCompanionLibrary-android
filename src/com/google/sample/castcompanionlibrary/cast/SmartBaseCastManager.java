...
/**
 * An abstract class that manages connectivity to a cast device. Subclasses are expected to extend the functionality of this class based on their
 * purpose.
 */
public abstract class SmartBaseCastManager
    implements DeviceSelectionListener, ConnectionCallbacks, OnConnectionFailedListener, OnFailedListener
{

  private static String CCL_VERSION;

  private static final String TAG = LogUtils.makeLogTag(SmartBaseCastManager.class);
  ...
  private final Set<IBaseCastConsumer> mBaseCastConsumers = Collections.synchronizedSet(new HashSet<IBaseCastConsumer>());
  ...
  protected static SmartBaseCastManager mCastManager;
  ...
  /**
   * A chance for the subclasses to perform what needs to be done when a route is unselected. Most of the logic is handled by the
   * {@link SmartBaseCastManager} but each subclass may have some additional logic that can be done, e.g. detaching data or media channels that they
   * may have set up.
   */
  abstract void onDeviceUnselected();

  /**
   * Since application lifecycle callbacks are managed by subclasses, this abstract method needs to be implemented by each subclass independently.
   *
   * @param device
   * @return
   */
  ...
  /**
   * Subclasses can decide how the Cast Controller Dialog should be built. If this returns <code>null</code>, the default dialog will be shown.
   *
   * @return
   */
  ...
  /**
   * Subclasses should implement this to react appropriately to the successful launch of their application. This is called when the application is
  ...
  protected SmartBaseCastManager(Context context, String applicationId)
  {
    ...
    Utils.saveStringToPreference(mContext, BaseCastManager.PREFS_KEY_APPLICATION_ID, applicationId);
  ...
  public static SmartBaseCastManager getCastManager()
  {
    return mCastManager;
  }

  /**
   * Sets the {@link Context} for the subsequent calls. Setting context can help the library to show error messages to the user.
   *
   * @param context
   */
  ...
  /**
   * This is called from {@link CastMediaRouterCallback} to signal the change in presence of cast devices on network.
   *
   * @param castPresent
   */
   ...
   *
   * @param stopAppOnExit
   *          If {@code true}, the application running on the cast device will be stopped when disconnected.
   * @param clearPersistedConnectionData
   *          If {@code true}, the persisted connection information will be cleared as part of this call.
   * @param setDefaultRoute
   *          If {@code true}, after disconnection, the selected route will be set to the Default Route.
   */
  public void disconnectDevice(boolean stopAppOnExit, boolean clearPersistedConnectionData, boolean setDefaultRoute)
  {
    ...
    {
      clearPersistedConnectionInfo(BaseCastManager.CLEAR_ALL);
  ...
  public void setDevice(CastDevice device)
  {
    ...
    if (null == mApiClient)
    {
      ...
      mApiClient = new GoogleApiClient.Builder(mContext).addApi(Cast.API, apiOptionsBuilder.build()).addConnectionCallbacks(this).addOnConnectionFailedListener(this).build();
      mApiClient.connect();
    }
  ...
  /**
   * Adds and wires up the Media Router cast button. It returns a pointer to the Media Router menu item if the caller needs such reference. It is
   * assumed that the enclosing {@link Activity} inherits (directly or indirectly) from {@link android.support.v7.app.ActionBarActivity}.
   *
   * @param menu
  ...
  /**
   * Adds and wires up the {@link android.support.v7.app.MediaRouteButton} instance that is passed as an argument. This requires that
   * <ul>
   * <li>The enclosing {@link Activity} inherits (directly or indirectly) from {@link android.support.v4.app.FragmentActivity}</li>
   * <li>User adds the {@link android.support.v7.app.MediaRouteButton} to the layout and pass a reference to that instance to this method</li>
   * <li>User is in charge of controlling the visibility of this button. However, this library makes it easier to do so: use the callback
   * <code>onCastAvailabilityChanged(boolean)</code> to change the visibility of the button in your client. For example, extend
   * {@link com.google.sample.castcompanionlibrary.cast.callbacks.VideoCastConsumerImpl} and override that method:
   * 
   * <pre>
   * {@code
   * public void onCastAvailabilityChanged(boolean castPresent) {
  ...
  /**
   * Calling this method signals the library that an activity page is made visible. In common cases, this should be called in the "onResume()" method
   * of each activity of the application. The library keeps a counter and when at least one page of the application becomes visible, the {@link
  ...
  /**
   * Calling this method signals the library that an activity page is made invisible. In common cases, this should be called in the "onPause()" method
   * of each activity of the application. The library keeps a counter and when all pages of the application become invisible, the {@link
   * onUiVisibilityChanged()} method is called.
   */
  ...
  /**
   * A utility method to validate that the appropriate version of the Google Play Services is available on the device. If not, it will open a dialog
   * to address the issue. The dialog displays a localized message about the error and upon user confirmation (by tapping on dialog) will direct them
   * to the Play Store if Google Play services is out of date or missing, or to system settings if Google Play services is disabled on the device.
   *
   * @param activity
   * @return
   */
  ...
  /**
   * Sets a flag to control whether disconnection form a cast device should result in stopping the running application or not. If <code>true</code> is
   * passed, then application will be stopped. Default behavior is not to stop the app.
   *
   * @param stopOnExit
   */
  ...
  /**
   * Returns the {@link RouteInfo} corresponding to the selected route.
   */
  ...
  /**
   * Sets the {@link RouteInfo} corresponding to the selected route.
   */
  ...
  /**
   * Turns on configurable features in the library. All the supported features are turned off by default and clients, prior to using them, need to
   * turn them on; it is best to do is immediately after initialization of the library. Bitwise OR combination of features should be passed in if
   ...
   * <li>FEATURE_LOCKSCREEN : turns on Lock Screen using {@link RemoteControlClient} in supported versions (JB+)
   * </ul>
   *
   * @param capabilities
   */
  ...
  /**
   * Clears the {@link Context}. Should be used when the client application is being destroyed to avoid context leak.
   */
  ...
  /**
   * Clears the {@link Context} if the current context is the same as the one provided in the argument <code>context</code>. Should be used when the
  ...
  /**
   * Returns <code>true</code> if there is enough persisted information to attempt a session recovery. For this to return <code>true</code>, there
   * needs to be persisted session ID and route ID from the last successful launch.
   *
   * @param context
   * @return
   */
  ...
  /**
   * Returns <code>true</code> if there is enough persisted information to attempt a session recovery. For this to return <code>true</code>, there
   * needs to be persisted session ID and route ID from the last successful launch. In addition, if <code>ssidName</code> is non-null, then an
   * additional check is also performed to make sure the persisted wifi name is the same as the <code>ssidName</code>
   *
   * @param context
   * @param ssidName
   * @return
  ...
  {
    String sessionId = Utils.getStringFromPreference(context, BaseCastManager.PREFS_KEY_SESSION_ID);
    String routeId = Utils.getStringFromPreference(context, BaseCastManager.PREFS_KEY_ROUTE_ID);
    String ssid = Utils.getStringFromPreference(context, BaseCastManager.PREFS_KEY_SSID);
  ...
  private void reconnectSessionIfPossibleInternal(RouteInfo theRoute)
  {
    ...
    String sessionId = Utils.getStringFromPreference(mContext, BaseCastManager.PREFS_KEY_SESSION_ID);
   ...
   * </ul>
   * Under these conditions, a best-effort attempt will be made to continue with the same session. This attempt will go on for
   * <code>timeoutInSeconds</code> seconds. During this period, an optional dialog can be shown if <code>showDialog</code> is set to <code>true</code>
   * . The message in this dialog can be changed by overriding the resource <code>R.string.session_reconnection_attempt</code>
   * <p>
   * Note: this variant does not check for the matching Wifi SSID name
   * </p>
   *
   * @param context
   * @param showDialog
   ...
   * @param timeoutInSeconds
   * @param ssidName
   *          The name of Wifi SSID
   */
  public void reconnectSessionIfPossible(final Context context, final boolean showDialog, final int timeoutInSeconds, String ssidName)
  {
   ...
   * </ul>
   * Under these conditions, a best-effort attempt will be made to continue with the same session. This attempt will go on for 5 seconds. During this
   * period, an optional dialog can be shown if <code>showDialog</code> is set to <code>true
   * </code>.
   *
   * @param context
   * @param showDialog
  ...
  /************************************************************/
  /**
   * This is called by the library when a connection is re-established after a transient disconnect. Note: this is not called by SDK.
   */
   ...
   * (non-Javadoc)
   * 
   * @see com.google.android.gms.GoogleApiClient.ConnectionCallbacks#onConnected (android.os.Bundle)
   */
  @Override
  public void onConnected(Bundle hint)
  {
    ...
    try
    {
      if (isFeatureEnabled(BaseCastManager.FEATURE_WIFI_RECONNECT))
      {
        String ssid = Utils.getWifiSsid(mContext);
        Utils.saveStringToPreference(mContext, BaseCastManager.PREFS_KEY_SSID, ssid);
      }
  ...
  /*
   * Note: this is not called by the SDK anymore but this library calls this in the appropriate time.
   */
   ...
   * (non-Javadoc)
   * 
   * @see com.google.android.gms.GoogleApiClient.OnConnectionFailedListener# onConnectionFailed(com.google.android.gms.common.ConnectionResult)
   */
  @Override
  ...
  /*
   * Launches application. For this succeed, a connection should be already established by the CastClient.
   */
      ...
      throws TransientNetworkDisconnectionException, NoConnectionException
  {
    ...
    if (mReconnectionStatus == ReconnectionStatus.IN_PROGRESS)
    {
      ...
      Cast.CastApi.joinApplication(mApiClient, mApplicationId, sessionId).setResultCallback(new ResultCallback<ApplicationConnectionResult>()
      {
        ...
        public void onResult(ApplicationConnectionResult result)
        {
          ...
          else
          {
            ...
            clearPersistedConnectionInfo(BaseCastManager.CLEAR_SESSION | BaseCastManager.CLEAR_MEDIA_END);
    ...
    else
    {
      ...
      Cast.CastApi.launchApplication(mApiClient, mApplicationId).setResultCallback(new ResultCallback<ApplicationConnectionResult>()
      {
  ...
  /*************************************************************/
  /**
   * Registers an {@link IBaseCastConsumer} interface with this class. Registered listeners will be notified of changes to a variety of lifecycle
  ...
  /**
   * Clears the persisted connection information. Bitwise OR combination of the following options should be passed as the argument:
   * <ul>
   * <li>CLEAR_SESSION</li>
   * <li>CLEAR_ROUTE</li>
   * <li>CLEAR_WIFI</li>
  ...
  public final void clearPersistedConnectionInfo(int what)
  {
    ...
    if (what == BaseCastManager.CLEAR_ALL || (what & BaseCastManager.CLEAR_SESSION) > 0)
    {
      Utils.saveStringToPreference(mContext, BaseCastManager.PREFS_KEY_SESSION_ID, null);
    }
    if (what == BaseCastManager.CLEAR_ALL || (what & BaseCastManager.CLEAR_ROUTE) > 0)
    {
      Utils.saveStringToPreference(mContext, BaseCastManager.PREFS_KEY_ROUTE_ID, null);
    }
    if (what == BaseCastManager.CLEAR_ALL || (what & BaseCastManager.CLEAR_WIFI) > 0)
    {
      Utils.saveStringToPreference(mContext, BaseCastManager.PREFS_KEY_SSID, null);
    }
    if (what == BaseCastManager.CLEAR_ALL || (what & BaseCastManager.CLEAR_MEDIA_END) > 0)
    {
      Utils.saveLongToPreference(mContext, BaseCastManager.PREFS_KEY_MEDIA_END, Long.MIN_VALUE);
    }
  }
  ...
  protected void startReconnectionService(long mediaDurationLeft)
  {
    ...
    Utils.saveLongToPreference(mContext.getApplicationContext(), BaseCastManager.PREFS_KEY_MEDIA_END, endTime);
