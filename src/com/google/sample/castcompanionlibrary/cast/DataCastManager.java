...
/**
 * A concrete subclass of {@link BaseCastManager} that is suitable for data-centric applications that use multiple namespaces.
 * <p>
 ...
 * way to get access to the singleton class is to call a variant of <code>getInstance()</code>. After initialization, callers can enable any available
 * feature (all features are off by default). To do so, call <code>enableFeature()</code> and pass an OR-ed expression built from one ore more of the
 * following constants:
 * <p>
 * <ul>
 * <li>FEATURE_DEBUGGING: to enable GMS level logging</li>
 * </ul>
 * Beyond managing the connectivity to a cast device, this class provides easy-to-use methods to send and receive messages using one or more
 * namespaces. These namespaces can be configured during the initialization as part of the call to <code>initialize()</code> or can be added later on.
 * Clients can subclass this class to extend the features and functionality beyond what this class provides. This class manages various states of the
 * remote cast device. Client applications, however, can complement the default behavior of this class by hooking into various callbacks that it
 * provides (see {@link IDataCastConsumer}). Since the number of these callbacks is usually much larger than what a single application might be
 * interested in, there is a no-op implementation of this interface (see {@link DataCastConsumerImpl}) that applications can subclass to override only
 * those methods that they are interested in. Since this library depends on the cast functionalities provided by the Google Play services, the library
 * checks to ensure that the right version of that service is installed. It also provides a simple static method
 * <code>checkGooglePlayServices()</code> that clients can call at an early stage of their applications to provide a dialog for users if they need to
 * update/activate their GMS library. To learn more about this library, please read the documentation that is distributed as part of this library.
 */
    ...
    implements Cast.MessageReceivedCallback
{
  ...
  /**
   * Initializes the DataCastManager for clients. Before clients can use DataCastManager, they need to initialize it by calling this static method.
   * Then clients can obtain an instance of this singleton class by calling {@link DataCastManager#getInstance()}. Failing to initialize this class
   * before requesting an instance will result in a {@link CastException} exception.
   * 
   * @param context
   * @param applicationId
   *          the unique ID for your application
   * @param namespaces
   *          to be set up for this class.
   * @return
   */
  ...
  /**
   * Returns the initialized instance of this class. If it is not initialized yet, a {@link CastException} will be thrown.
   * 
   * @see initialze()
   * @return
  ...
  /**
   * Returns the initialized instance of this class. If it is not initialized yet, a {@link CastException} will be thrown. The {@link Context} that is
   * passed as the argument will be used to update the context. The main purpose of updating context is to enable the library to provide
   * {@link Context} related functionalities, e.g. it can create an error dialog if needed. This method is preferred over the similar one without a
   * context argument.
   * 
  ...
  /**
   * Adds a channel with the given <code>namespace</code> and registers {@link DataCastManager} as the callback receiver. If the namespace is already
   * registered, this returns <code>false</code>, otherwise returns <code>true
   * </code>.
   * 
   * @param namespace
   * @return
  ...
  /**
   * Unregisters a namespace. If namespace is not already registered, it returns <code>false</code>, otherwise a successful removal returns <code>true
   * </code>.
   * 
   * @param namespace
   * @return
  ...
  /**
   * Sends the <code>message</code> on the data channel for the <code>namespace</code>. If fails, it will call <code>onMessageSendFailed</code>
   * 
   * @param message
   * @param namespace
   * @throws NoConnectionException
   ...
   *           If framework is still trying to recover from a possibly transient loss of network
   * @throws IllegalArgumentException
   *           If the the message is null, empty, or too long; or if the namespace is null or too long.
   ...
   * 
   * @throws TransientNetworkDisconnectionException If framework is still trying to recover from a possibly transient loss of network
   * 
  ...
  /*************************************************************/
  /**
   * Registers an {@link IDataCastConsumer} interface with this class. Registered listeners will be notified of changes to a variety of lifecycle and
  ...
  public void addDataCastConsumer(IDataCastConsumer listener)
  {
    if (null != listener)
    {
      ...
      {
        LOGD(TAG, "Adding Listener " + listener + " was already registered, " + "skipping this step");
      }
    }
  }
